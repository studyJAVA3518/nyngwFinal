...
public class CommonJoinMemberVO {
	...
	public JoinMemberVO toJoinMemberVO(){
		JoinMemberVO vo = new JoinMemberVO();
		vo.setDept_name(dept_name);
		vo.setMem_addr1(mem_addr1);
		vo.setMem_addr2(mem_addr2);
		vo.setMem_dept_number(mem_dept_number);
		vo.setMem_email(mem_email);
		vo.setMem_id(mem_id);
		vo.setMem_name(mem_name);
		vo.setMem_number(mem_number);
		vo.setMem_position_number(mem_position_number);
		vo.setMem_pwd(mem_pwd);
		vo.setPosition_name(position_name);
		vo.setMem_zip(mem_zip);
		vo.setMem_tel(mem_tel);
		vo.setMem_retirement(mem_retirement);
		vo.setMem_reg(mem_reg);
		vo.setMem_pwd(mem_pwd);
		return vo;
	}
